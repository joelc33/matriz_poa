...
if($co_proyecto_alcance!=''||$co_proyecto_alcance!=null){
	$sql = "SELECT * FROM t38_proyecto_alcance WHERE co_proyecto_alcance=".$co_proyecto_alcance;
	$result = $comunes->ObtenerFilasBySqlSelect($sql);
	foreach($result as $key => $row){
		$data6 = json_encode(array(
			"co_proyecto_alcance"     => trim($row["co_proyecto_alcance"]),
...
this.co_vinculo_proyecto = new Ext.form.ComboBox({
	...
	typeAhead: true,
	valueField: 'tx_vinculo_proyecto',
...
getStoreCO_VINCULO_PROYECTO:function(){
    this.store = new Ext.data.JsonStore({
        url:'formulacion/modulos/proyecto/funcion.php?op=15',
        root:'data',
        fields:[
