...
nuevoProyecto.main = {
	...
	this.tabuladores = new Ext.Panel({
		...
		border: false,
		padding	: 10,
		html: '<center><p><i><b>Ejercicio Fiscal Año:</b> <?php echo $_SESSION['ejercicio_fiscal'];?></i></p><p><i><b>Estado:</b> Cerrado.</i></p><p><i>No se pueden crear nuevos proyectos para este Periodo Fiscal.</i></p></center>',
	});
	this.tabuladores.render('contenedornuevoProyecto');
	}
}
...
this.id_ejecutor = new Ext.form.ComboBox({
	...
	allowBlank:false,
		...
		nuevoProyecto.main.id_ejecutor.setValue(record.data.id_ejecutor);
		nuevoProyecto.main.id_tab_ejecutor.setValue(record.data.co_ejecutores);
		this.collapse();
        }
});
...
this.guardar = new Ext.Button({
	...
	//nuevoProyecto.main.panel.remove(tabuladorDos.main.panelDatos);
		...
		direccionar.load({ url: 'formulacion/modulos/proyecto/nuevoProyecto.php', scripts: true, text: 'Cargando...'/*,              params:'codigo='+action.result.c*/});
             }
        });
...
<?php if( in_array( array( 'de_privilegio' => 'proyecto.crear', 'in_habilitado' => true), $_SESSION['spe_session'][0][0] )){ ?>
    buttonAlign:'left',
    buttons:[
        this.guardar
    ]
