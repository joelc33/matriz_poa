...
if($co_proyecto_localizacion!=''||$co_proyecto_localizacion!=null){
	$sql = "SELECT * FROM t33_proyecto_localizacion WHERE co_proyecto_localizacion=".$co_proyecto_localizacion;
	$result = $comunes->ObtenerFilasBySqlSelect($sql);
	foreach($result as $key => $row){
		$data3 = json_encode(array(
			"co_proyecto_localizacion"     => trim($row["co_proyecto_localizacion"]),
			"id_proyecto"     => trim($row["id_proyecto"]),
			"co_ambito"     => trim($row["co_ambito_localizacion"]),
...
this.nuevo = new Ext.Button({
    ...
    handler:function(){
        ...
        this.msg.load({
         url:"formulacion/modulos/proyecto/nuevaLocalizacion.php",
         scripts: true,
...
this.eliminar= new Ext.Button({
	...
	if(boton=="yes"){
        Ext.Ajax.request({
            method:'POST',
            url:'formulacion/modulos/proyecto/funcion.php?op=18',
            params:{
                co_proyecto_localizacion_nacional:tabuladorTres.main.gridPanel_.getSelectionModel().getSelected().get('co_proyecto_localizacion_nacional')
            },
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    autoHeight:true,
    tbar:[
<?php if( in_array( array( 'de_privilegio' => 'proyecto.ambito.agregar', 'in_habilitado' => true), $_SESSION['spe_session'][0][0] )){ ?>
        this.nuevo,
<?php } ?>
	'-',
<?php if( in_array( array( 'de_privilegio' => 'proyecto.ambito.quitar', 'in_habilitado' => true), $_SESSION['spe_session'][0][0] )){ ?>
	this.eliminar
<?php } ?>
    ],
    columns: [
...
this.nuevoComunal = new Ext.Button({
    ...
    handler:function(){
        ...
        this.msg = Ext.get('formulario_ubicacion');
        this.msg.load({
         url:"formulacion/modulos/proyecto/nuevaComuna.php",
         scripts: true,
	 params: {id_proyecto:editarProyecto.main.id_proyecto.getValue()},
         text: "Cargando.."
        });
...
this.eliminarComunal= new Ext.Button({
	...
	if(boton=="yes"){
        Ext.Ajax.request({
            method:'POST',
            url:'formulacion/modulos/proyecto/funcion.php?op=20',
            params:{
                co_proyecto_localizacion_comunal:tabuladorTres.main.gridPanelComunal_.getSelectionModel().getSelected().get('co_proyecto_localizacion_comunal')
            },
            success:function(result, request ) {
                obj = Ext.util.JSON.decode(result.responseText);
                if(obj.success==true){
...
this.gridPanelComunal_ = new Ext.grid.GridPanel({
    ...
    autoHeight:true,
    tbar:[
<?php if( in_array( array( 'de_privilegio' => 'proyecto.localizacion.agregar', 'in_habilitado' => true), $_SESSION['spe_session'][0][0] )){ ?>
        this.nuevoComunal,
<?php } ?>
	'-',
<?php if( in_array( array( 'de_privilegio' => 'proyecto.localizacion.quitar', 'in_habilitado' => true), $_SESSION['spe_session'][0][0] )){ ?>
	this.eliminarComunal
<?php } ?>
    ],
    columns: [
    new Ext.grid.RowNumberer(),
...
getStoreCO_AMBITO:function(){
    this.store = new Ext.data.JsonStore({
        url:'formulacion/modulos/proyecto/funcion.php?op=10',
        root:'data',
        fields:[
...
getListaUbicacion: function(){
    this.store = new Ext.data.JsonStore({
    url:'formulacion/modulos/proyecto/funcion.php?op=17',
    root:'data',
    fields:[
...
getListaComuna: function(){
    this.store = new Ext.data.JsonStore({
    url:'formulacion/modulos/proyecto/funcion.php?op=19',
    root:'data',
    fields:[
