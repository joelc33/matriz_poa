...
$comunes = new ConexionComun();
	...
	if($_POST['BuscarBy']=="true"){
		$sql.="SELECT nu_codigo as id_proy_ac, t52.de_nombre, tx_ejecutor, fe_inicio, fe_fin, mo_ac as mo_proyecto, 
			coalesce(mo_calculado, 0) as mo_registrado, '2' as co_tipo 
			FROM ac_seguimiento.tab_ac as t46
			inner join mantenimiento.tab_ejecutores as t24 on t46.id_tab_ejecutores=t24.id_ejecutor 
			inner join mantenimiento.tab_ac_predefinida as t52 on t52.id = t46.id_tab_ac_predefinida
			where t46.nu_codigo = '".decode($_POST['id_proyecto'])."' AND t46.in_activo is true";
	}
		$sql.=" LIMIT 2";
	...
	if($co_tipo==1){
		...
		$url_proy_ac='formulacion/modulos/avance_ac/meta_proyecto.php';
		$op='1';
...
this.panel.render("consultarAccionSAAC");
...
resultadoSAC.main = {
	...
	this.tabuladores.render('consultarAccionSAAC');
	}
}
Ext.onReady(resultadoSAC.main.init, resultadoSAC.main);
</script>
