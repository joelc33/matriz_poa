...
if($_GET['op']==1){
	...
	$sql = "SELECT t47.*, t52.nombre as nb_ac, de_unidad_medida as tx_unidades_medida, t24.tx_ejecutor, ('PG' || t24b.id_ejecutor || id_ejercicio || lpad(t46.id_accion::text, 5, '0')) as id_ac, t53.nu_numero as numero, t53.de_nombre as nombre, mo_ac_ae_meta(t47.id_tab_t47_ac_accion_especifica) AS mo_cargado, t24b.tx_ejecutor as ejecutor_resp FROM t47_ac_accion_especifica as t47
	left join mantenimiento.tab_unidad_medida as t21 on t47.id_unidad_medida=t21.id
	...
	foreach($result as $key => $row){
		$data[] = array(
		    ...
		    "tx_codigo"     => trim($row["numero"]),
		    "descripcion"     => trim($row["nombre"]),
		    "objetivo_institucional"     => trim($row["objetivo_institucional"]),
		    "meta"     => trim($row["meta"]),
		    "ponderacion"     => trim($row["ponderacion"]),
		    "bien_servicio"     => trim($row["bien_servicio"]),
		    "total"     => trim($row["monto"]),
