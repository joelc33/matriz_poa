...
if($codigo!=''||$codigo!=null){
	...
	foreach($result as $key => $row){
		...
		$id_pac = trim($row["co_proyecto_acc_espec"]);
	}
}else{
	$data = json_encode(array(
...
<?php }} ?>
	width:400,
	maxLength: 140,
	autoCreate: {tag: "input", type: "text", autocomplete: "off", maxlength: 140},
	allowBlank:false,
        listeners:{
            change: function(){
...
this.guardar = new Ext.Button({
    ...
    handler:function(){
        ...
        importarEditar.main.formPanel_.getForm().submit({
            method:'POST',
            url:'formulacion/modulos/accionEspecifica/funcion.php?op=2',
