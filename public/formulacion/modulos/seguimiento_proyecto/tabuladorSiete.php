...
this.cargarAccion= new Ext.Button({
	...
	tabuladorSiete.main.mascara.show();
        ...
        this.msg.load({
         url:"formulacion/modulos/seguimiento_proyecto_ae/editar.php",
...
this.eliminar= new Ext.Button({
	...
	if(boton=="yes"){
        Ext.Ajax.request({
            ...
            params:{
                co_proyecto_acc_espec:tabuladorSiete.main.gridPanelAccion_.getSelectionModel().getSelected().get('co_proyecto_acc_espec'), op: 4
            },
...
this.cargarPartida = new Ext.Button({
	...
	tabuladorSiete.main.mascara.show();
	 ...
	 params:{codigo:'<?php echo $id_proyecto; ?>'},
         url:"formulacion/modulos/seguimiento_proyecto_ae/cargar.php",
         scripts: true,
         text: "Cargando.."
        });
    }
...
this.verFisico = new Ext.Button({
	...
	iconCls: 'icon-reporteest',
	handler: function(boton){
		addTab(tabuladorSiete.main.gridPanelFisica_.getSelectionModel().getSelected().get('co_proyecto_acc_espec'),'Proyecto '+tabuladorSiete.main.gridPanelFisica_.getSelectionModel().getSelected().get('id_proyecto')+', Accion Especifica '+tabuladorSiete.main.gridPanelFisica_.getSelectionModel().getSelected().get('tx_codigo'),'formulacion/modulos/seguimiento_proyecto/partida.php','load','icon-reporteest','codigo='+tabuladorSiete.main.gridPanelFisica_.getSelectionModel().getSelected().get('co_proyecto_acc_espec'));
		/*var direccionar = Ext.get('contenedortabuladorSiete');
		direccionar.load({ url: 'formulacion/modulos/proyecto/editarProyecto.php', scripts: true, text: 'Cargando...',              params:'codigo='+tabuladorSiete.main.gridPanel_.getSelectionModel().getSelected().get('co_proyectos')});*/
	}
});
...
this.gridPanelAccion_ = new Ext.grid.GridPanel({
    ...
    autoWidth: true,
    tbar:[
        this.cargarAccion,'-',this.editarAccion,'-',this.eliminar,'-',this.cargarPartida,'-',this.verDetalle
    ],
    columns: [
...
this.editarFisico= new Ext.Button({
	...
	tabuladorSiete.main.mascara.show();
	 ...
	 method:'POST',
	 params:{codigo:this.codigo},
         url:"formulacion/modulos/seguimiento_proyecto_ae/editar_fisico.php",
         scripts: true,
         text: "Cargando.."
        });
    }
...
this.gridPanelFisica_ = new Ext.grid.GridPanel({
    ...
    autoWidth: true,
    tbar:[
        this.verFisico,'-',this.editarFisico
    ],
    columns: [
    new Ext.grid.RowNumberer(),
