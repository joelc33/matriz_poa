...
if($codigo!=''||$codigo!=null){
	$sql = "SELECT * FROM proyecto_seguimiento.tab_proyecto WHERE id = ".$codigo;
	...
	foreach($result as $key => $row){
		$data1 = json_encode(array(
			...
			"co_estatus_proyecto"     => trim($row["id_tab_estatus_proyecto"]),
			"codigo_new_etapa"     => trim($row["co_new_etapa"]),
			"fecha_inicio"     => trim($row["fe_inicio"]),
			"fecha_fin"     => trim($row["fe_fin"]),
			"tx_objetivo_general"     => trim($row["de_objetivo"]),
			"tx_descripcion_proyecto"     => trim($row["de_proyecto"]),
			"co_situacion_presupuestaria"     => trim($row["id_tab_situacion_presupuestaria"]),
			"mo_total"     => trim($row["mo_proyecto"]),
	...
	$id_proyecto = trim($row["nu_codigo"]);
	$id_ejercicio = trim($row["id_tab_ejercicio_fiscal"]);
	$co_estatus = trim($row["id_tab_estatus"]);
	}
...
Ext.ns("editarProyectoS");
function formatoNumero(val){
	    return paqueteComunJS.funcion.getNumeroFormateado(val);
return val;
};
editarProyectoS.main = {
...
this.co_sector = new Ext.form.ComboBox({
	...
	listeners:{
            change: function(){
                editarProyectoS.main.storeCO_SUB_SECTOR.load({
...
if(this.OBJ.co_sector){
	this.storeCO_SUB_SECTOR.load({
		...
		callback: function(){editarProyectoS.main.co_sub_sector.setValue(editarProyectoS.main.OBJ.co_sub_sector);}
	});
}
...
this.guardar = new Ext.Button({
    ...
    handler:function(){
	//editarProyectoS.main.panel.remove(tabuladorDos.main.panelDatos);
        if(!editarProyectoS.main.formulario.getForm().isValid()){
            Ext.Msg.alert("Alerta","Debe ingresar los campos en rojo");
            return false;
        }
        editarProyectoS.main.formulario.getForm().submit({
            method:'POST',
		...
		var direccionar = Ext.get('contenedoreditarProyectoS<?php echo $codigo ?>');
		direccionar.load({ url: 'formulacion/modulos/seguimiento_proyecto/editar.php', scripts: true, text: 'Cargando...',              params:'codigo='+action.result.c});
             }
        });
...
this.formulario.render("contenedoreditarProyectoS<?php echo $codigo ?>");
},
...
Ext.onReady(editarProyectoS.main.init, editarProyectoS.main);
</script>
<div id="contenedoreditarProyectoS<?php echo $codigo ?>"></div>
<div id="formulario_ubicacionS"></div>
