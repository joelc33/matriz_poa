<?php
$sql = "SELECT id, id_tab_proyecto, mime_proyecto, nb_archivo_proyecto, mime_satelital, nb_archivo_satelital FROM proyecto_seguimiento.tab_proyecto_imagen WHERE id_tab_proyecto='".$id_proyecto."'";
$result = $comunes->ObtenerFilasBySqlSelect($sql);
foreach($result as $key => $row){
	$data4 = json_encode(array(
		"co_proyecto_imagen"     => trim($row["id"]),
...
this.imagenProyecto = new Ext.form.FieldSet({
    border:false,
    html:'<img width="250" height="155" src="modulos/imagen/funcion.php?op=3&id_proyecto='+tabuladorCuatro.main.OBJ.id_proyecto+'&t='+new Date().getTime()+'">'
});
...
this.imagenSatelite = new Ext.form.FieldSet({
    border:false,
    html:'<img width="250" height="155" src="modulos/imagen/funcion.php?op=4&id_proyecto='+tabuladorCuatro.main.OBJ.id_proyecto+'&t='+new Date().getTime()+'">'
});
