...
$comunes = new ConexionComun();
	...
	if($_POST['BuscarBy']=="true"){
		$sql.="SELECT nu_codigo as id_proy_ac, t52.de_nombre, tx_ejecutor, fe_inicio, fe_fin, mo_ac,
	coalesce(mo_calculado, 0) as monto_calc, '2' as co_tipo
	...
	where nu_codigo = '".decode($_POST['id_proyecto'])."' AND t46.in_activo is true order by 1 DESC";
	...
	foreach($result as $key => $row){
		$data = json_encode(array(
			    ...
			    "monto"     => trim($row["mo_ac"]),
			    "mo_registrado"     => trim($row["monto_calc"]),
		));
	$co_tipo = $row["co_tipo"];
	}
	if($co_tipo==1){
		...
		$url_proy_ac='formulacion/modulos/seguimiento_ae_actividad/meta_proyecto.php';
		$op='1';
		$variable="+'&id_proyecto='+resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')";
	}else{
		$datosEnunciado='Accion Centralizada';
		$fieldDatos='Datos de la Accion Centralizada';
		$co_proy_ac='Codigo de la Accion Centralizada';
		$url_proy_ac='formulacion/modulos/seguimiento_ac_actividad/meta_proyecto.php';
		$op='1';
		$variable="+'&id_accion_centralizada='+resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')";
	}
?>
...
this.accionFisica = new Ext.Button({
	...
	handler: function(boton){
		addTab(resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')+'ae','<?php echo $datosEnunciado;?>:'+resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')+' >  Accion Especifica:'+resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('tx_codigo'),'<?php echo $url_proy_ac;?>','load','icon-accion_especifica','codigo='+resultadoSAC.main.gridPanel_.getSelectionModel().getSelected().get('co_proyecto_acc_espec')<?php echo $variable;?>);
	}
});
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    autoScroll:true,
    stateful: true,
    listeners:{cellclick:function(Grid, rowIndex, columnIndex,e ){resultadoSAC.main.accionFisica.enable();}},
...
this.panel.render("consultarAccionSAC");
...
resultadoSAC.main.accionFisica.disable();
});
...
Ext.onReady(resultadoSAC.main.init, resultadoSAC.main);
</script>
<?php     
}else{
?>
...
resultadoSAC.main = {
	...
	this.tabuladores.render('consultarAccionSAC');
	}
}
Ext.onReady(resultadoSAC.main.init, resultadoSAC.main);
</script>
