...
if($codigo!=''||$codigo!=null){
	$sql = "SELECT * FROM ac_seguimiento.tab_meta_fisica
	where id=".$codigo;
...
this.nuevo = new Ext.Button({
	...
	handler: function(boton){
		paqueteComunJS.funcion.mostrarVentana({url:'formulacion/modulos/seguimiento_ac_actividad/meta_financiera.php?id_proyecto='+metaEditarPR.main.id_proyecto.getValue()+'&id_tab_ac_ae='+metaEditarPR.main.co_proyecto_acc_espec.getValue(),parametro:'no'});
	}
});
...
this.eliminar= new Ext.Button({
	...
	handler: function(boton){
		metaEditarPR.main.eliminarRequerimiento();
	}
});
...
this.store_lista =  new Ext.data.GroupingStore({
	reader: new Ext.data.JsonReader({fields:metaEditarPR.main.Registro})
});
...
if(this.OBJ.co_metas!='')
{
	...
	this.store_lista.load({
		params: {co_metas:metaEditarPR.main.co_metas.getValue()},
	});
}
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    autoScroll:true,
    stateful: true,
    listeners:{cellclick:function(Grid, rowIndex, columnIndex,e ){metaEditarPR.main.eliminar.enable();}}
});
...
this.guardar = new Ext.Button({
    ...
    handler:function(){
        ...
        if(!metaEditarPR.main.formPanel_.getForm().isValid()){
            Ext.Msg.alert("Alerta","Debe ingresar los campos en rojo");
            return false;
        }
//*****Array del Grid********//
	listado = paqueteComunJS.funcion.getJsonByObjStore({
		store:metaEditarPR.main.gridPanel_.getStore()
	});
	metaEditarPR.main.JsonDetalle.setValue(listado);
//**************************//
        metaEditarPR.main.formPanel_.getForm().submit({
            method:'POST',
            url:'formulacion/modulos/seguimiento_ac_actividad/funcion.php',
            ...
            success: function(form, action) {
			 ...
			 animEl: document.body,
                 ...
                 metaEditarPR.main.winformPanel_.close();
             }
        });
...
eliminarRequerimiento:function(){
                var s = metaEditarPR.main.gridPanel_.getSelectionModel().getSelections();
                for(var i = 0, r; r = s[i]; i++){
                      metaEditarPR.main.store_lista.remove(r);
                }
...
Ext.onReady(metaEditarPR.main.init, metaEditarPR.main);
</script>
