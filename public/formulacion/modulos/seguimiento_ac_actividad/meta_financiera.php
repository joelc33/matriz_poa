<?php        
	$data = json_encode(array(
		...
		"id_tab_ac_ae"     => $_GET['id_tab_ac_ae'],
		"co_estado"     => 23,
		"co_municipio"     => "",
		"co_parroquia"     => "",
	));
...
Ext.ns("detallemetaEditarPR");
detallemetaEditarPR.main = {
init:function(){
...
this.co_estado = new Ext.form.ComboBox({
	...
	listeners:{
            change: function(){
                detallemetaEditarPR.main.storeCO_MUNICIPIO.load({
...
if(this.OBJ.co_estado){
	this.storeCO_MUNICIPIO.load({
		...
		callback: function(){detallemetaEditarPR.main.co_municipio.setValue(detallemetaEditarPR.main.OBJ.co_municipio);}
	});
}
...
this.co_municipio = new Ext.form.ComboBox({
	...
	listeners:{
            change: function(){
                detallemetaEditarPR.main.storeCO_PARROQUIA.load({
...
this.storeCO_PARTIDA.load({
		params: {id_proyecto:this.OBJ.id_proyecto, id_tab_ac_ae: this.OBJ.id_tab_ac_ae}
	});
	paqueteComunJS.funcion.seleccionarComboByCo({
...
this.guardar = new Ext.Button({
	...
	handler:function(){
	if(detallemetaEditarPR.main.formPanel_.form.isValid()){
		var e = new metaEditarPR.main.Registro({
			co_municipio:detallemetaEditarPR.main.co_municipio.getValue(),
			tx_municipio:detallemetaEditarPR.main.co_municipio.getRawValue(),
			co_parroquia:detallemetaEditarPR.main.co_parroquia.getValue(),
	    		tx_parroquia:detallemetaEditarPR.main.co_parroquia.getRawValue(),
			mo_presupuesto:detallemetaEditarPR.main.mo_presupuesto.getValue(),
			co_partida:detallemetaEditarPR.main.co_partida.getValue(),
			co_fuente_financiamiento:detallemetaEditarPR.main.co_fuente_financiamiento.getValue(),
	    		tx_fuente_financiamiento:detallemetaEditarPR.main.co_fuente_financiamiento.getRawValue()
		});
		var cant = metaEditarPR.main.store_lista.getCount();
			(cant==0)?0:metaEditarPR.main.store_lista.getCount()+1;

			metaEditarPR.main.store_lista.insert(cant, e);
			metaEditarPR.main.gridPanel_.getView().refresh();
			detallemetaEditarPR.main.winformPanel_.close();
	}else{
		Ext.Msg.show({
			title:'Mensaje',
...
getStoreCO_ESTADO:function(){
    this.store = new Ext.data.JsonStore({
        url:'formulacion/modulos/seguimiento_ac_actividad/funcion.php',
        root:'data',
	baseParams: {
		op: 7
	},
...
Ext.onReady(detallemetaEditarPR.main.init, detallemetaEditarPR.main);
</script>
