...
$comunes = new ConexionComun();
	...
	if($co_tipo==1){
		...
		$op='1';
		$variable="+'&id_proyecto='+resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')";
	}else{
		...
		$op='2';
		$variable="+'&id_accion_centralizada='+resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('id_accion_centralizada')";
	}
?>
<script type="text/javascript">
Ext.ns("resultadoPRAC");
...
this.accionFisica = new Ext.Button({
	...
	handler: function(boton){
		addTab(resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')+'ae','<?php echo $datosEnunciado;?>:'+resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('id_proyecto')+' >  Accion Especifica:'+resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('tx_codigo'),'<?php echo $url_proy_ac;?>','load','icon-accion_especifica','codigo='+resultadoPRAC.main.gridPanel_.getSelectionModel().getSelected().get('co_proyecto_acc_espec')<?php echo $variable;?>);
	}
});
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    autoScroll:true,
    stateful: true,
    listeners:{cellclick:function(Grid, rowIndex, columnIndex,e ){resultadoPRAC.main.accionFisica.enable();}},
...
this.panel.render("consultarAccionSP");
...
resultadoPRAC.main.accionFisica.disable();
});
...
Ext.onReady(resultadoPRAC.main.init, resultadoPRAC.main);
</script>
<?php     
}else{
?>
...
resultadoPRAC.main = {
	...
	this.tabuladores.render('consultarAccionSP');
	}
}
Ext.onReady(resultadoPRAC.main.init, resultadoPRAC.main);
</script>
