...
this.nuevo = new Ext.Button({
    ...
    handler:function(){
        ...
        this.msg.load({
         url:"{{ URL::to('mantenimiento/partida/nuevo') }}",
         scripts: true,
         text: "Cargando.."
        });
    }
...
this.editar= new Ext.Button({
	...
	partidaLista.main.mascara.show();
        this.msg = Ext.get('formulariopartida');
        this.msg.load({
         url:"{{ URL::to('mantenimiento/partida/editar') }}/"+this.codigo,
         scripts: true,
         text: "Cargando.."
        });
    }
...
this.eliminar= new Ext.Button({
	...
	if(boton=="yes"){
        Ext.Ajax.request({
            method:'POST',
            url:'{{ URL::to('mantenimiento/partida/eliminar') }}',
            params:{
		_token: '{{ csrf_token() }}',
...
this.habilitar= new Ext.Button({
	...
	if(boton=="yes"){
        Ext.Ajax.request({
            method:'POST',
            url:'{{ URL::to('mantenimiento/partida/habilitar') }}',
            params:{
		_token: '{{ csrf_token() }}',
                id: partidaLista.main.gridPanel_.getSelectionModel().getSelected().get('id')
            },
...
this.buscador = new Ext.form.TwinTriggerField({
	...
	onTrigger1Click : function() {
		...
		partidaLista.main.store_lista.baseParams._token = '{{ csrf_token() }}';
		partidaLista.main.store_lista.load();
	},
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    autoHeight:true,
    tbar:[
			@if( in_array( array( 'de_privilegio' => 'partidas.nuevo', 'in_habilitado' => true), Session::get('credencial') ))
			  this.nuevo,'-',
			@endif
			@if( in_array( array( 'de_privilegio' => 'partidas.editar', 'in_habilitado' => true), Session::get('credencial') ))
				this.editar,'-',
			@endif
			@if( in_array( array( 'de_privilegio' => 'partidas.habilitar', 'in_habilitado' => true), Session::get('credencial') ))
				this.habilitar,'-',
			@endif
			@if( in_array( array( 'de_privilegio' => 'partidas.deshabilitar', 'in_habilitado' => true), Session::get('credencial') ))
				this.eliminar,'-',
			@endif
				this.buscador
    ],
    ...
    listeners:{cellclick:function(Grid, rowIndex, columnIndex,e ){
			partidaLista.main.editar.enable();
			partidaLista.main.habilitar.enable();
			partidaLista.main.eliminar.enable();
		}},
...
partidaLista.main.editar.disable();
partidaLista.main.habilitar.disable();
partidaLista.main.eliminar.disable();
});
...
getLista: function(){
    ...
    url:'{{ URL::to('mantenimiento/partida/storeLista') }}',
    root:'data',
    fields:[
    {name: 'id'},
    {name: 'co_partida'},
    ...
    {name: 'nombre',
        convert: function(v, r) {
            return r.co_partida + ' - ' + r.tx_nombre;
        }
    }
           ]
    });
