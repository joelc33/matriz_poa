...
Ext.ns('parametroUbicacionAC');
parametroUbicacionAC.main = {
init: function(){
...
this.co_municipio = new Ext.form.ComboBox({
	...
	listeners:{
            change: function(){
                parametroUbicacionAC.main.storeCO_PARROQUIA.load({
...
this.botones = new this.GrupoBotones({
				...
				bbar: [{
						...
						items: [
							...
							@endif
							@if( in_array( array( 'de_privilegio' => 'ubicacion.ac.todos', 'in_habilitado' => true), Session::get('credencial') ))
								{
							...
							@endif
							@if( in_array( array( 'de_privilegio' => 'ubicacion.ac.exportar.municipio', 'in_habilitado' => true), Session::get('credencial') ))
								{
							...
							@endif
							@if( in_array( array( 'de_privilegio' => 'ubicacion.ac.exportar.todo', 'in_habilitado' => true), Session::get('credencial') ))
								{
...
this.formpanel = new Ext.form.FormPanel({
	...
	title: 'UBICACION - ACCION CENTRALIZADA',
	items:[
		this.botones
	]
});

this.formpanel.render('parametroUbicacionAC');
},
onImprimir : function() {
if(!parametroUbicacionAC.main.formpanel.getForm().isValid()){
    Ext.Msg.alert("Alerta","Debe ingresar los campos en rojo");
    return false;
}
	bajar.load({
		url: '{{ URL::to('reporte/poa/ac/ubicacion') }}?'+parametroUbicacionAC.main.formpanel.getForm().getValues(true)
	});
},
...
if(!parametroUbicacionAC.main.formpanel.getForm().isValid()){
	...
	bajar.load({
		url: '{{ URL::to('reporte/poa/ac/ubicacion/exportar') }}?'+parametroUbicacionAC.main.formpanel.getForm().getValues(true)
	});
},
...
onLimpiar: function(){
    parametroUbicacionAC.main.formpanel.getForm().reset();
},
...
Ext.onReady(parametroUbicacionAC.main.init, parametroUbicacionAC.main);
</script>
<div id="parametroUbicacionAC"></div>
