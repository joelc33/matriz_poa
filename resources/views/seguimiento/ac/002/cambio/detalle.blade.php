...
this.datos2 +='<p class="registro_detalle"><b>Accion Especifica: </b>'+this.OBJ.de_nombre+'</p>';
this.datos2 +='<p class="registro_detalle"><b>Actividad: </b>'+this.OBJ.nb_meta+'</p>';
...
this.cerrar = new Ext.Button({
  ...
  handler:function(){
		addTab('foma001'+this.OBJ.nu_codigo,'A.C: '+this.OBJ.nu_codigo,'{{ URL::to('ac/seguimiento/002/datos') }}/'+this.OBJ.id,'load','icon-editar','');
	}
});
...
this.formPanel_ = new Ext.form.FormPanel({
  ...
  tbar:[
		@if( in_array( array( 'de_privilegio' => 'acseguimiento.001.cambio.editar', 'in_habilitado' => true), Session::get('credencial') ))
