...
this.nuevo = new Ext.Button({
	...
	handler: function(boton){
		...
		this.msg.load({
		 url:"{{ URL::to('ac/seguimiento/003/actividad/financiera/nuevo') }}/{{ $data->id }}",
		 scripts: true,
		 text: "Cargando.."
		});
	}
...
this.gridPanel_ = new Ext.grid.GridPanel({
    ...
    listeners:{cellclick:function(Grid, rowIndex, columnIndex,e ){
			forma004ActividadEditar.main.editar.enable();
                        if(forma004ActividadEditar.main.gridPanel_.getSelectionModel().getSelected().get('id_tab_origen')>1){
...
this.winformPanel_ = new Ext.Window({
    title:'FORMA 3 : METAS FINANCIERAS',
    modal:true,
    constrain:true,
width:814,
    frame:true,
...
,getLista: function(){
    this.store = new Ext.data.JsonStore({
    url:'{{ URL::to('ac/seguimiento/003/actividad/financiera/storeLista') }}/{{ $data->id }}',
    root:'data',
    fields:[
    {name: 'id'},
    {name: 'id_tab_origen'},
